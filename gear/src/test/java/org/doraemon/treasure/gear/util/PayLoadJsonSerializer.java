...
public class PayLoadJsonSerializer extends JsonInstanceSerializer<ServiceProfile> {
    ...
    public ServiceInstance<ServiceProfile> deserialize(byte[] bytes) throws Exception {
        ...
        ServiceInstanceBuilder<ServiceProfile> builder =
                ServiceInstance.<ServiceProfile>builder().address(rawServiceInstance.getAddress()).id(rawServiceInstance.getId()).name(rawServiceInstance.getName())
                        .port(rawServiceInstance.getPort()).registrationTimeUTC(rawServiceInstance.getRegistrationTimeUTC()).serviceType(rawServiceInstance.getServiceType())
                        .sslPort(rawServiceInstance.getSslPort() == null ? 0 : rawServiceInstance.getSslPort()).uriSpec(rawServiceInstance.getUriSpec());
        if (rawServiceInstance.getPayload() instanceof Map) {
            builder.payload(new ServiceProfile((Map<String, Object>) rawServiceInstance.getPayload()));
    ...
    private ServiceInstance<Object> deserialize0(byte[] bytes) throws JsonParseException, JsonMappingException, IOException {
