package org.doraemon.treasure.ares.util;
...
public class SQLParser {
    ...
    private static Pattern ptruncate_table = Pattern.compile("\\s+table\\s+([a-z0-9_@\\.\"$]+)\\s+");
    ...
    public static Pair<String, String> findTableNameAndType(String sql0) {
        ...
        if (sql0.startsWith("truncate")) {
            Matcher m = ptruncate_table.matcher(sql0);
            if (m.find(8)) {
                return Pair.of(m.group(1), TableNameHandler.SQL_TYPE_TRUNCATE);
            }
            return null;
        }
