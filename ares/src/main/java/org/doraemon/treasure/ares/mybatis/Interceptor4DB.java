...
import org.doraemon.treasure.ares.datasource.threadlocal.DataSourceContextHolder;
import org.doraemon.treasure.ares.util.Pair;
import org.doraemon.treasure.ares.util.ReflectionUtils;
import org.doraemon.treasure.ares.util.SQLParser;
import org.doraemon.treasure.ares.util.StringUtils;
import org.doraemon.treasure.ares.util.Validate;
...
public class Interceptor4DB implements Interceptor {
    ...
    public void setTableNameVsHandler(Properties properties) {
        ...
        for (Entry<Object, Object> entry : properties.entrySet()) {
            Validate.isTrue(entry.getKey() instanceof String, "不接受的参数,表名必须为String,%s", entry.getKey());

            String tableName = (String) entry.getKey();
            Object tableNameHandler = entry.getValue();
            if (tableNameHandler instanceof String) {
                try {
                    Class<?> c = Class.forName((String) tableNameHandler);
                    tableNameHandler = c.newInstance();
                }
                catch (Throwable ex) {
                    Validate.isTrue(false, ex.toString());
                }

            }

            Validate.isTrue(tableNameHandler instanceof TableNameHandler);
            tableNameVsHandler.put(tableName, (TableNameHandler) tableNameHandler);
        }
    }
}
